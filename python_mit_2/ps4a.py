...
def getWordScore(word, n):
    ...
    """
    score = 0
    # Loop through each letter in word.  Add points to score
    for c in word:
        score += SCRABBLE_LETTER_VALUES[c]
    score *= len(word)
    # If hand size equals the word length, add additional 50 points
    if n == len(word):
        score += 50
    return score
...
def updateHand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    # Create a new hand dictionary copy
    new_hand = dict(hand)
    # Loop through each letter in word
    for c in word:
        # If letter in hand, subtract one count from value
        if c in new_hand:
...
def isValidWord(word, hand, wordList):
    ...
    for c in word:
        if c in new_hand and new_hand[c] > 0:
            new_hand[c] = new_hand.get(c, 0) - 1
        else:
            letters_in_hand = False
    # Check if both conditions are true
    if word_in_list and letters_in_hand:
        return True
    else:
        return False
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print("\"" + user_word + "\"" + " earned " + str(getWordScore(user_word, n)) + " points. Total: " +
...
def playGame(wordList):
    ...
    """
    user_input = ""
    # Play as long as the user do not enter "e"
    while user_input != "e":
